...
11. Als editor die finale Ausgabe schreiben: JEDER Artikel bekommt ein Artikelbild (image-Block: file, alt, caption, credit), nicht mehr nur Leitartikel und große Meldungen. Bilder NIEMALS als SVG-Zeichnung. Jedes Bild wird eigens für seinen Artikel über den ChatGPT-Bild-Workflow neu erzeugt (claude-in-chrome): keine Wiederverwendung von Bestandsbildern aus public/images/, und zwei Artikel derselben Ausgabe dürfen nie dieselbe Datei referenzieren — auch nicht bei wiederkehrenden Themen. Prompt je Bild mit "New unrelated image." beginnen, die konkrete Szene der Geschichte beschreiben und die Stil-Formel anhängen ("photorealistic editorial news photograph, 3:2 landscape … Cinematic documentary photography, natural light, muted desaturated grade, shallow depth of field, ultra sharp. No legible text, no numbers, no brand logos", bei Länder-/Politikmotiven zusätzlich "no flags"); danach aus chatgpt.com/library herunterladen, mit sips -Z 1400 -s format jpeg -s formatOptions 82 konvertieren und unter sprechendem, motivspezifischem deutschem Dateinamen ablegen. Ist die Neuerzeugung nicht möglich (kein Browser), das fehlende Bild explizit in der Abschlusszusammenfassung melden statt es kommentarlos wegzulassen oder ersatzweise ein Bestandsbild einzusetzen. Nur lokale Dateien, nie SVG als Bild. JSON nach content/editions/${slug}.json (edition.number = ${nextNumber}, edition.date = "${date}", edition.slot = "${slot}", edition.isExample = false, edition.generatedAt = aktuelle ISO-Zeit, edition.dataAsOf = ehrliche Beschreibung des Datenstands, edition.schemaVersion = 2). Zusätzlich das neue Top-Level-Feld "lernecke" bauen (3 bis 5 Einträge, unabhängig von und zusätzlich zu den bestehenden kurzen learningNotes-Einzeilern, die unverändert bleiben): jeder Eintrag mit disziplin ("VWL" | "BWL" | "Politikwissenschaft"), modell (Name eines echten akademischen Modells/Konzepts, z. B. Phillips-Kurve, Prinzipal-Agent-Theorie, Median-Wähler-Theorem, Zinsparität, Efficient Market Hypothesis, Komparativer Kostenvorteil), erklaerung (die Theorie selbst von Grund auf für Laien erklärt, mehrere Sätze/ein kurzer Absatz, mind. 280 Zeichen) und anwendung (konkrete Anwendung dieses Modells auf ein Ereignis DIESER Ausgabe, mind. 140 Zeichen) — dabei die von macro-analyst und equity-analyst geflaggten Modelle/Zusammenhänge aufgreifen und wie ein Team aus VWL-, BWL- und Politikwissenschaftsprofessoren argumentieren, nicht nur einen Satz hinwerfen.
