...
11. Als editor die finale Ausgabe schreiben: JEDER Artikel bekommt ein Artikelbild (image-Block: file, alt, caption, credit), nicht mehr nur Leitartikel und große Meldungen. Bilder NIEMALS als SVG-Zeichnung. public/images/ ist eine wachsende Bildbank wiederverwendbarer, bewusst generischer Motive (Zentralbank, Chip-Reinraum, Containerhafen, Handelsraum, Rechenzentrum, Devisenhandel, Ölmeerenge, Goldbarren, Bankfassade, Fabrikfließband u. a.) — bei wiederkehrenden Themen zuerst ein Bestandsbild mit neuer, artikelspezifischer Caption wiederverwenden (normale redaktionelle Praxis, kein Kompromiss); nur wenn kein Bestandsbild passt, ein neues, bewusst breit wiederverwendbares Motiv über den ChatGPT-Bild-Workflow erzeugen (claude-in-chrome), danach mit sips auf ca. 1400px verkleinern. Ist weder Bestandsbild noch Neuerzeugung möglich, das fehlende Bild explizit in der Abschlusszusammenfassung melden statt es kommentarlos wegzulassen. Nur lokale Dateien, nie SVG als Bild. JSON nach content/editions/${slug}.json (edition.number = ${nextNumber}, edition.date = "${date}", edition.slot = "${slot}", edition.isExample = false, edition.generatedAt = aktuelle ISO-Zeit, edition.dataAsOf = ehrliche Beschreibung des Datenstands, edition.schemaVersion = 2). Zusätzlich das neue Top-Level-Feld "lernecke" bauen (3 bis 5 Einträge, unabhängig von und zusätzlich zu den bestehenden kurzen learningNotes-Einzeilern, die unverändert bleiben): jeder Eintrag mit disziplin ("VWL" | "BWL" | "Politikwissenschaft"), modell (Name eines echten akademischen Modells/Konzepts, z. B. Phillips-Kurve, Prinzipal-Agent-Theorie, Median-Wähler-Theorem, Zinsparität, Efficient Market Hypothesis, Komparativer Kostenvorteil), erklaerung (die Theorie selbst von Grund auf für Laien erklärt, mehrere Sätze/ein kurzer Absatz, mind. 280 Zeichen) und anwendung (konkrete Anwendung dieses Modells auf ein Ereignis DIESER Ausgabe, mind. 140 Zeichen) — dabei die von macro-analyst und equity-analyst geflaggten Modelle/Zusammenhänge aufgreifen und wie ein Team aus VWL-, BWL- und Politikwissenschaftsprofessoren argumentieren, nicht nur einen Satz hinwerfen.
